...
/**
 * Filtro que restringe el acceso a las páginas de alta y edición de empleados a
 * usuarios con roles 'ADMIN' o 'RRHH'. Si el usuario no tiene una sesión activa
 * o no tiene los permisos adecuados, se le redirige al login o al panel
 * principal con un mensaje de error.
 */
@WebFilter(urlPatterns = {"/gestor/altaEmpleados.html", "/gestor/editorEmpleados.html", })
public class FiltroRRHH implements Filter {

	/**
	 * Método que intercepta las peticiones a las URLs protegidas. Verifica si el
	 * usuario tiene sesión activa y si tiene el rol adecuado ('ADMIN' o 'RRHH')
	 * para acceder a las páginas. Si el usuario no cumple los requisitos, se le
	 * redirige al login o al panel principal.
	 *
	 * @param request  la solicitud HTTP recibida
	 * @param response la respuesta HTTP que se enviará
	 * @param chain    la cadena de filtros que se debe ejecutar si el usuario tiene
	 *                 acceso
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if ("ADMIN".equals(u.getRol()) || "RRHH".equals(u.getRol())) {
			...
			// Si el rol no es ADMIN o RRHH, se redirige al panel principal con un mensaje
			// de error
